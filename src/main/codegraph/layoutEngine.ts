 ...
 *
 * Takes a serialized CodeGraph and computes {x, y} positions for each node.
 * Uses BFS depth levels to position nodes in rows — single O(V+E) pass.
 *
 * Supports incremental layout: when new nodes are added, computes only new
 * positions and shifts siblings minimally.
 *
 ...
 * Uses BFS depth levels to arrange nodes in columns.
 */
export function computeLayout(
  ...
  // If no positions, set reasonable defaults for bounds
 ...
 * Preserves existing positions and only computes for new ones.
 */
export function computeIncrementalLayout(
  ...
  // Count existing nodes at each depth for offset
  const existingDepthCounts = new Map<number, number>()
  for (const pos of existingPositions) {
    const count = existingDepthCounts.get(pos.depth) ?? 0
    existingDepthCounts.set(pos.depth, count + 1)
  }
  ...
  for (const [depth, nodes] of depthBuckets) {
    ...
    // Find the max Y of existing nodes at this depth
